document.getElementById("btn-signup-submit").onclick = function() {
    if (document.getElementById('input-password').value != document.getElementById('input-password-confirm').value) {
    ...
    if (!document.getElementById('chkbox-terms-n-conditions').checked) {
        document.getElementById('chkbox-terms-n-conditions').setCustomValidity('You must agree with our terms and conditions.');
    } else {
        // input is valid -- reset the error message
