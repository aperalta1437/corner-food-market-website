...
CREATE OR REPLACE FUNCTION set_initial_state_for_ITEM_CATEGORY() RETURNS TRIGGER AS $BODY$
BEGIN
    NEW.IS_LEAF = TRUE;
...
CREATE TRIGGER set_initial_state_on_before_INSERT
BEFORE INSERT ON ITEM_CATEGORY
FOR EACH ROW
EXECUTE PROCEDURE set_initial_state_for_ITEM_CATEGORY();
...
CREATE OR REPLACE FUNCTION set_initial_state_for_ITEM_IMAGE() RETURNS TRIGGER AS $BODY$
BEGIN
    NEW.FILE_NAME = CONCAT(CAST(NEW.ID AS VARCHAR), '.', NEW.FILE_EXTENSION);
    NEW.CREATED_AT = now();
    RETURN NEW;
END; $BODY$
LANGUAGE plpgsql;
...
FOR EACH ROW
EXECUTE PROCEDURE set_initial_state_for_ITEM_IMAGE();
...
CREATE OR REPLACE FUNCTION set_CREATED_AT_for_EXCEPTION_LOG() RETURNS TRIGGER AS $BODY$
BEGIN
    NEW.CREATED_AT = now();
    RETURN NEW;
END; $BODY$
...
FOR EACH ROW
EXECUTE PROCEDURE set_CREATED_AT_for_EXCEPTION_LOG();


-- BANNER_IMAGE ----------------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION set_initial_state_for_BANNER_IMAGE() RETURNS TRIGGER AS $BODY$
BEGIN
    NEW.FILE_NAME = CONCAT(CAST(NEW.ID AS VARCHAR), '.', NEW.FILE_EXTENSION);
    NEW.CREATED_AT = now();
    RETURN NEW;
...
CREATE TRIGGER set_initial_state_on_before_INSERT
BEFORE INSERT ON BANNER_IMAGE
FOR EACH ROW
EXECUTE PROCEDURE set_initial_state_for_BANNER_IMAGE();
